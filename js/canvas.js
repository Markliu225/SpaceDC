...
const oC = document.getElementById('orbitCanvas'),    oX = oC ? oC.getContext('2d') : null;
const dC = document.getElementById('detailCanvas'),   dX = dC ? dC.getContext('2d') : null;
...
function resizeAll() {
  [sC, rC, tC].forEach(c => {
    if (c && c.offsetWidth > 0 && c.offsetHeight > 0) { c.width = c.offsetWidth; c.height = c.offsetHeight; }
  });
}
