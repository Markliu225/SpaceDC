...
6. **Six hull architectures** — each design is a genuinely different satellite shape (`twin_params.json` `architecture`): the classic single-truss, a 24-blade twin-truss tower, ISS-style ribbon blanket wings, the LUMID smallsat with integrated cross panels, a parabolic-dish comms hull with windmill wings, and the Redwire-style flat payload bay (its own row of discrete GPU modules faces the sun side, each independently clickable → GPU panel) with flat deck-parallel GaAs wings bolted onto the bay-edge lugs — added panels chain outward one by one — and boom radiators off the deck faces. Physics areas follow the architecture.
