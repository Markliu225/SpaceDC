...
15. **Satellite builder — pick a platform, then build it** — the Twin page opens on a four-step wizard (`backend/satellite_assets.py` + the *Build* chip to reopen it) that specs a satellite from scratch: **① Platform** — four vendor buses, each a genuinely different hull with its own payload bay (**SpaceX Compute Bus**, truss, 12 rack slots · **Redwire Payload Bay**, flat bay, 7 GPU modules · **Sophia Space TILE**, a 1 m² panel with a solar face and a radiating back, 6 slots · **Ada Space Compute Node**, a networked smallsat with twin wings and a 100 Gbps laser mesh, 8 slots — the two platforms whose geometry we do not have are drawn on their card from the vendor's published reference design rather than rendered from the stand-in hull they fly), shipping the flight-checked loadout of the audited design preset for that hull; **② Structure** — the power chain (cells, wing segments, chemistry, pack), the thermal chain (coating, panel span/ratio) and the pointing mode, with panel sizes **typed as real dimensions** rather than T-shirt tiers (segments per side, radiator span in metres, aspect ratio — clamped to what the generator and the physics accept) and the area each one produces updating live; **③ Payload** — a GPU **per bay slot**, painted with a card brush: leave slots empty to fly lighter, or mix models, and the regenerated USD really does render only the fitted blades, each wearing its card-type marker bar; **④ Workload** — the job schedule. Nothing touches the flying satellite until *Run*: every edit instead re-runs a backend **dry-run** (`POST /satellite_build/preview`) that commissions the draft on a *detached* engine and returns the same derived stats and per-schedule fit verdicts the live panels use — so the design check you read before Run is the one you get after it. *Run* is a single atomic `POST /satellite_build` (hull + loadout + slots + schedule together, USD regen, Kit layer reload). A mixed bay is modelled honestly: each card type is its own tensor-parallel group and the satellite aggregates over them (`GpuJobDetail.mix`), so clicking blade 5 in the viewport reports *that* card's operating point.
