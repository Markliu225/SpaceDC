...
13. **轨道与星座设计器** —— Overview 页右栏可视化设计星座：编辑轨道六要素（a·e·i·Ω·ω·M，实时显示当前星座的六要素读数）与 Walker 构型（轨道面数 × 每面卫星数、相位因子），*Apply* 后后端合成参考 TLE（`POST /orbit_design`）并激活设计——三维舰队、覆盖地图、Kit 星座环、物理引擎跟踪星全部即时重新传播，设计过程所见即所得。可选的**新加坡地面站**：在地球上标红站址（覆盖地图 + Kit 地球），并显示其与所设计星座的实时通信可见性（每 tick 的可见星数 + 最佳仰角，仰角掩模模型），以及一整轨的过境分析（`GET /ground_visibility`）：接触窗口、覆盖占比、下次过境倒计时。
