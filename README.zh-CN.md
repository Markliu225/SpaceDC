...
- **Satellite Twin** —— 进入即是**卫星选型/搭建向导**（选型 → 结构设计 → 载荷设计 → workload，功能 14）；Run 之后才是单星视图：Omniverse 视口、Configurator（算力 · 太阳翼 · 散热 · 电池 · 可展开结构 · 姿态）、实时遥测折线条，以及 Build / Designs / Compare 面板。
...
15. **卫星选型与搭建向导** —— Twin 页进入即是一个四步向导（`backend/satellite_assets.py`，关闭后可由顶栏 *Build* 入口重新打开），从零把一颗卫星配出来：**① 选型** —— 四种厂商平台，每种都是**真正不同的星体构型**且自带槽位数（**SpaceX Compute Bus** 桁架 12 槽 · **Redwire Payload Bay** 扁平载荷舱 7 个 GPU 模块 · **Sophia Space TILE** 向阳面发电、背阴面散热的 1 m² 平板瓦片 6 槽 · **Ada Space Compute Node** 双翼组网小卫星、100 Gbps 激光星间链路 8 槽——后两家没有厂商几何，选型卡上按其**公开 reference design 手绘**，而不是渲染我们的替身 hull），出厂即带该构型下已审计设计预设的配置（物理校验全部通过）；**② 结构设计** —— 电（电池片/翼段数/化学体系/电池包）、热（涂层/散热板尺寸与长宽比）与轨道指向模式，**板面尺寸按真实量纲手动输入**（翼段数、散热板跨度 m、长宽比，越界自动钳位到生成器与物理接受的范围），面积随手实时推导；**③ 载荷设计** —— **逐槽位**选 GPU 型号，用"卡型画笔"点选槽位：可以留空以更轻更凉，也可以混插不同型号，重新生成的 USD **只渲染装了卡的刀片**，每片还带一条卡型色标；**④ Workload** —— 作业档案。**按 Run 之前不碰在线卫星**：每次编辑只触发后端**试算**（`POST /satellite_build/preview`）——在一个**独立引擎实例**上把草稿"装配"出来，返回与在线面板同源的派生指标与逐档案适配判定，所以 Run 前看到的设计校验就是 Run 后的结果。Run 是**一次原子** `POST /satellite_build`（构型 + 配置 + 槽位 + 作业档案一起落地，USD 重生成，Kit 重载图层）。混插也如实建模：每种卡型自成一个张量并行组、整星按组求和（`GpuJobDetail.mix`），在视口里点第 5 片刀片读到的就是**那张卡**的工作点。

16. **电池化学与电池包配置** —— *Battery* 区可选化学体系（Li-ion / LiFePO₄ / Li-S / 固态 —— 各自对应一组质量能量密度 Wh/kg 与往返效率）与电池包尺寸档位（S/M/L/XL，单位 kg）。容量是**推导量** —— 电池包质量 × 能量密度，实时以 kWh 显示 —— 且往返损耗只在充电一侧计一次（放电 1 : 1），因此表中的效率是真正的往返效率。两个旋钮都会改变进出地影的续航故事，都是 Compare 的对比维度，六个设计预设也都带上了这两个字段。
