 ...
 *   - The flat inset background visible while the stream is connecting or
 *     failed.
 *   - Top-left status chip: OMNIVERSE ● STREAMING (ok) /
 *                            OMNIVERSE ● CONNECTING (info) /
 *                            LOCAL ● FALLBACK (warn).
 *   - Bottom-right compass/axis gizmo (64x64 svg).
 *   - Selected satellite accent ring + crosshair marker (overlay).
 *
...
export function EarthViewport({ fallback = 'overview' }: { fallback?: 'overview' | 'twin' }) {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden rounded-lg border border-border-weak bg-bg-inset shadow-card">
      ...
      <div className="absolute left-3 top-3 z-10 flex items-center gap-2 rounded-full border border-border-weak bg-bg-app/70 px-2.5 py-1 text-[10px] uppercase tracking-[0.14em] backdrop-blur">
        <Dot
          color={mode === 'streaming' ? colors.ok : mode === 'connecting' ? colors.accent : colors.warn}
          size={7}
          pulse={mode !== 'fallback'}
        />
...
function PlaceholderEarth({ mode, msg }: { mode: 'connecting' | 'fallback'; msg: string }) {
  void mode; void msg
  return (
    <div className="absolute inset-0 grid place-items-center bg-bg-inset">
      <div className="text-[11px] uppercase tracking-[0.18em] text-text-lo animate-live-pulse">
        loading scene…
      </div>
    </div>
  )
...
function CompassGizmo() {
  return (
    <svg
      ...
      aria-hidden="true"
    >
      <circle cx="0" cy="0" r="22" fill="none" stroke={colors.text.lo} strokeOpacity="0.5" strokeWidth="0.5" />
      <circle cx="0" cy="0" r="2" fill={colors.text.hi} />
      <line x1="0" y1="-22" x2="0" y2="-26" stroke={colors.text.hi} strokeWidth="1.4" />
      <line x1="0" y1="22"  x2="0" y2="26"  stroke={colors.text.lo} strokeWidth="1" />
      <line x1="-22" y1="0" x2="-26" y2="0" stroke={colors.text.lo} strokeWidth="1" />
      <line x1="22"  y1="0" x2="26"  y2="0" stroke={colors.text.lo} strokeWidth="1" />
      <text x="0" y="-12" textAnchor="middle" fontSize="6" fill={colors.text.hi} fontFamily={typography.mono}>N</text>
    </svg>
  )
}
