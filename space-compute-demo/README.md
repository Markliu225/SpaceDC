...
6. **Six hull architectures** — each design is a genuinely different satellite shape (`twin_params.json` `architecture`): the classic single-truss, a 24-blade twin-truss tower, ISS-style ribbon blanket wings, the LUMID smallsat with integrated cross panels, a parabolic-dish comms hull with windmill wings, and the Redwire-style flat payload bay (GPUs inside the deck, avionics row on the sun face) with generated GaAs wings off the bay edges and boom radiators off the deck faces. Physics areas follow the architecture.
