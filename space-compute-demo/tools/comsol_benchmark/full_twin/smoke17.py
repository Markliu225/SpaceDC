...
rs = comp.physics('otl').prop('RadiationSettings')
print('vf props:', {k: str(rs.getString(k)) for k in ('radiationResolution','radiationMethod','viewFactorsUpdateTolerance','storeViewFactors','viewFactorUpdateThreshold')}, flush=True)
rs.set('radiationResolution', '32'); short('W2_res32', True)
rs.set('viewFactorsUpdateTolerance', '-1'); short('W3_res32_tolm1', True)
rs.set('radiationMethod', 'dirarea'); short('W4_dirarea', True)
