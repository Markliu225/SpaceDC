...
def render_persp(meshes, out, eye, target, focal=35.0, hap=20.955, vap=15.2908,
                 size=1100, by_part=True):
    """Perspective render matching USD Camera semantics (focalLength + film
    aperture) so camera framing can be checked for real. Output aspect =
    hap/vap; the full frustum is drawn so margins read true."""
    ...
    img.save(out)
    print(f"[persp] {out}  {W}x{H}  focal={focal}  tris={len(faces)}")


def render(path, out, view="iso", size=1100, by_part=True, label=False, only=None,
           eye=None, target=None, persp=False, focal=35.0):
    ...
    if persp and eye is not None and target is not None:
        render_persp(meshes, out, eye, target, focal=focal, size=size, by_part=by_part)
        return
...
if __name__ == "__main__":
    ...
    render(src, out, view=view, size=size, label=label, only=only, eye=eye,
           target=target, persp=persp, focal=focal)
